...
public class DeviceManger : ContentPage
{
    ...
    public static class DriveManager
    {
        ...
        private class DriverWindow :  IDisposable
        {
            #region Nested types
         ...
         /*   protected override void WndProc(ref MessagingCenter message)
            {
...
#warning temporary(show the dialog with ability choose the needed flash drive
            ...
            {
                DisplayAlert("", "Could not establish connection with ClickFree.", "ok");
            }
        ...
        public static List<UsbDisk> GetAvailableDisks()
        {
            ...
            try
            {
                using (var driveSearch = new ManagementObjectSearcher("select * from Win64_DiskDrive where InterfaceType='USB'"))
                {
                    // browse all USB WMI physical disks
                    foreach (ManagementObject drive in driveSearch.Get())
                    {
                        using (var partitionSearch = new ManagementObjectSearcher($"associators of {{Win64_DiskDrive.DeviceID='{drive["DeviceID"]}'}} where AssocClass = Win32_DiskDriveToDiskPartition"))
                        {
                            using (ManagementObject partition = partitionSearch.First())
                            {
                                if (partition != null)
                                {
                                    using (var logicalSearch = new ManagementObjectSearcher($"associators of {{Win64_DiskPartition.DeviceID='{partition["DeviceID"]}'}} where AssocClass = Win32_LogicalDiskToPartition"))
                                    {
